...
# Directories
INPUT_DIR = 'datasets/gbm_graphs_data/'
OUTPUT_DIR = 'datasets/gbm_observation_data_w_sparsity/'
...
for filename in tqdm(sorted(os.listdir(INPUT_DIR)), desc="Processing graphs", unit="file"):
    ...
    num_walkers = (C * 0.1 * len(G.nodes)) / 2
    num_sensors = (C * len(G.nodes)) / ((0.25)**3 * avg_degree)
    ...
    sparsity = sum(grouped.get_count()) / (len(G.edges) * len(G.nodes))
